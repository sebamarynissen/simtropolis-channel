...
If one of the uploaded .zip folders contains an empty `metadata.yaml` file, then the channel will generate the following metadata for it automatically:
...
It is strongly recommended to explicitly specify the subfolder you want your plugin to end up in. This can be done by specifying a `metadata.yaml` file
```yaml
subfolder: 660-parks
```
...
You may wish to split up your package in both a *models & props* ("*resources*") part, and *lots* part, which is useful if you expect other people to create re-lots using your props and models. Reference your assets in the `metadata.yaml` as follows:
...
Note the asset ids of `${{ assets.0.assetId }}` these are interpolated strings, and will be automatically replaced when the metadata is created with the name of the first asset.
These placeholders or variables can be useful for specifying the name of some thing when you do not quite yet know exactly what value it will have.
...
A variant is an option a user needs to make when installing a mod.
...
variants:
  - variant: { nightmode: standard }
    assets:
      - assetId: author-plugin-name-maxisnite
  - variant: { nightmode: dark }
    dependencies: [ "simfox:day-and-nite-mod" ]
    assets:
      - assetId: author-plugin-name-darknite
...
---
assetId: author-plugin-name-darknitenite
url: https://community.simtropolis.com/...
version: 1.0.0
lastModified: ...
```
...
While it is strongly discouraged for new files, this approach is supported on the channel too.
...
For example, consider a .zip with the following file and folder structure
...
Another common option encountered with plugins are separate growable lots for players using the CAM vs players without CAM.
...
variants:
  - variant: { driveside: right }
    assets:
      - assetId: plugin-name-rhd
variants:
  - variant: { driveside: left }
    assets:
      - assetId: plugin-name-left
```
...
variants:
  - variant: { group:name:resolution: sd }
    assets:
      - assetId: plugin-name
      - assetId: plugin-name-sd
  - variant: { group:name:resolution: hd }
    assets:
      - assetId: plugin-name
      - assetId: plugin-name-hd
```
...
If your package contains custom variants specific to your package , like choosing a model color or a texture option, then you will have to write the variant metadata yourself.
...
However, as you can derive from above, the most common cases are covered, so as long as you stick to the conventions as explained, you should need to write minimal metadata to handle variants.

### Advanced Customization: Interpolation Algorithm

If your plugin has complex requirements, you may wish to define the metadata structure yourself, while leaving certain items 'blank' to be autogenerated when the metadata is submitted.
This is useful if packages have very complex metadata where the standard patch algorithm might not be able to generate what you want, which is annoying if you have no easy way to debug what will be generated eventually.

Using this "fill in the blank" approach, the algorithm will use the structure you have created, and then fill in the "blanks" using the variables you define with their autogenerated values.
To use this feature, prepend your metadata with a config object that specifies the algorithm to be used:
...
> [!WARNING]
> This approach should be used sparingly, as you **must** specify **all** required metadata yourself, but it can be useful for some extreme mods where the metadata deviates considerably from what would've been auto-generated.
...
On top of that, you must also link your account on Simtropolis with your GitHub username in both the `permissions.yaml` and `lint-config.yaml` files in this repository.
If you want to start developing DLL plugins, create a PR which adds the required data to those files.
